...
export class CreateTrackComponent extends Destroyable {
    ...
    protected generateTrack(): void {
        this.generatedTrack.set(TrackBuilder.designTrack(this.form.value.name!.toLocaleLowerCase(), this.form.value.style!));
    }

    protected saveTrack(): void {
        this.trackService.addTrack$(this.generatedTrack()!).pipe(
    ...
    protected checkSimilarTrack(name: string, style: TrackStyles): void{
        this.trackService.isTrackAvailable$(name.toLocaleLowerCase(), style).pipe(
