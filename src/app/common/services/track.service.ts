...
import { from, last, map, Observable, of, switchMap, tap } from 'rxjs';
...
export class TrackService {
    ...
    public getTrackRecords$(trackId: number): Observable<StockableRecord[]> {
        return from(liveQuery(() => RETROSKI_DB.records.where({'trackId': trackId}).sortBy('timing'))).pipe(
            map(records => records.map(record => new StockableRecord(record.trackId, record.player, record.date, record.timing)))
        );
    }
