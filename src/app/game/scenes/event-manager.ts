...
import type { EventConfig } from '../models/event-config';
import type { RaceResult } from '../models/race-result';
import { format } from 'date-fns';
import { Config } from '../config';
import type { TrackManager } from '../utils/track-manager';
...
export class EventManager extends Scene {
    ...
    public prepareManager(eventConfig: EventConfig): void {
        ...
        this.lastResultsContainer.innerHTML =
            this.prepareLastResultsTable(eventConfig) || '<div class="placeholder">No result for the moment</div>';
        this.actualRankingContainer.innerHTML =
            this.prepareActualRankingTable(eventConfig) || '<div class="placeholder">No ranking for the moment</div>';
        ...
        return eventConfig.racesResults
            ...
            .map(raceResult => {
                const skier1RecordPosition = raceResult.skier1Timing
                    ? ` (${this.trackManager.getRecordPosition(raceResult.trackName, raceResult.skier1Timing)})`
                    : '';
                const skier2RecordPosition = raceResult.skier2Timing
                    ? ` (${this.trackManager.getRecordPosition(raceResult.trackName, raceResult.skier2Timing)})`
                    : '';
