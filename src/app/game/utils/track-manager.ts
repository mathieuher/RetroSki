...
import type { StockableGhost } from '../models/stockable-ghost';
import type { StockableRecord } from '../models/stockable-record';
import { StockableTrack } from '../models/stockable-track';
import type { Track } from '../models/track';
import type { TrackStyles } from '../models/track-styles.enum';
...
export class TrackManager {
        ...
        for (const track of Config.DEFAULT_TRACKS) {
            ...
            fetch(`assets/tracks/${track}.json`)
                ...
                ?.then((track: StockableTrack) => {
                    if (!storedTrack?.builderVersion || (track.builderVersion || 0) > storedTrack.builderVersion)
    ...
    public saveRecord(track: Track, record: StockableRecord): GlobalResult | null {
        ...
        return new GlobalResult(
            this.getCurrentRecords(track.name),
            track.records.filter(r => r.timing < record.timing).length + 1
        );
    }
