...
export class EventManager extends Scene {
    ...
    private prepareLastResultsTable(eventConfig: EventConfig): string {
        return eventConfig.racesResults.filter(raceResult => raceResult.isStarted()).map(raceResult => {
            const skier1RecordPosition = `${raceResult.skier1Timing ? ' (' + this.trackManager.getRecordPosition(raceResult.trackName, raceResult.skier1Timing) + ')' : ''}`;
            const skier2RecordPosition = `${raceResult.skier2Timing ? ' (' + this.trackManager.getRecordPosition(raceResult.trackName, raceResult.skier2Timing) + ')' : ''}`
    ...
    private prepareNextRacesTable(eventConfig: EventConfig): string {
        return eventConfig.racesResults.filter(raceResult => !raceResult.isCompleted()).map((raceResult, index) => {
